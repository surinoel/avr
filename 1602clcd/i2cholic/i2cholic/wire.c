...
void Wire_begin_slave(uint8_t address)
{
...
static uint8_t requestFrom(uint8_t address, uint8_t quantity, uint8_t sendStop)
{
...
size_t Wire_write_len(const uint8_t *data, size_t quantity)
{
  size_t i;
  if(transmitting){
  // in master transmitter mode
